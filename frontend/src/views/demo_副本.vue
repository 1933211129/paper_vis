  ...
  computed: {
    conclusionItems() {
      ...
      if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object' && data[0] !== null) {
        return Object.entries(data[0]).map(([key, value]) => ({ name: key, content: value }))
      }
      return []
    },
    innovationItems() {
  ...
  methods: {
    ...
    async loadData() {
      try {
        ...
        const [metadataRes, abstractRes, contextRes, methodologyRes, resultsRes, conclusionRes, innovationRes, figureMapRes] = await Promise.all([
          fetch('/data/metadata.json').then(r => r.json()),
          fetch('/data/abstract.json').then(r => r.json()),
          fetch('/data/context_related_work.json').then(r => r.json()),
          fetch('/data/methodology_setup.json').then(r => r.json()),
          fetch('/data/results_analysis.json').then(r => r.json()),
          fetch('/data/conclusion.json').then(r => r.json()),
          fetch('/data/innovation_opportunities.json').then(r => r.json()),
          fetch('/data/figure_map.json').then(r => r.json())
        ])
        
        this.metadata = metadataRes
        this.abstractData = abstractRes
        this.contextData = contextRes
        this.methodologyData = methodologyRes
        this.resultsData = resultsRes
        this.conclusionData = conclusionRes
        this.innovationData = innovationRes
        this.figureMap = figureMapRes
    ...
    getFiguresForSection(sectionName) {
      ...
      return this.figureMap[sectionName].map(fig => ({
        ...fig,
        imageSrc: `/data/images/${fig.figure_id || 'default'}.jpg`
      }))
    },
